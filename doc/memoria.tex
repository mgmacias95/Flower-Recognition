...
Para tamaño $k=20$ en clustering, tenemos las siguientes tasas de acierto en la \hyperref[res20SIFT]{Tabla \ref*{res20SIFT}}.

\begin{table}[H]
  \begin{tabular}{l|r|r}
    \multirow{2}{*}{\textbf{Algoritmo}} & \multicolumn{2}{c}{\textbf{Tasa de acierto}} \\
    \cline{2-3}
    & \textbf{SIFT} & \textbf{SURF} \\
    \hline
    \textit{SVM one-vs-all} & $\qquad$ 0.63 & $\qquad$ 0.45 \\ 
    \textit{SVM one-vs-one} & $\qquad$ 0.63 & $\qquad$ 0.45 \\
    \textit{RF con bootsting} & $\qquad$ 0.65 & $\qquad$ 0.45 \\
    \textit{RF sin bootsting} & $\qquad$ 0.59 & $\qquad$ 0.63 \\
  \end{tabular}
  \label{res20SIFT}
  \caption{Tasa de aciertos de los algoritmos usando sólo la geometría de la imagen. $k=20$ con SIFT.}
\end{table}
...
\begin{table}[H]
  \begin{tabular}{l|r}
    ...
    \hline
    \textit{SVM one-vs-all} & $\qquad$ 0.45 \\
    \textit{SVM one-vs-one} & $\qquad$ 0.45 \\
    \textit{RF con bootsting} & $\qquad$ 0.45 \\
    \textit{RF sin bootsting} & $\qquad$ 0.63 \\
  \end{tabular}
  \label{res20SURF}
  \caption{Tasa de aciertos de los algoritmos usando sólo la geometría de la imagen. $k=20$ con SURF.}
\end{table}

\subsection{Imágenes con procesamiento \textit{Color Quantization}}

Para tamaño $k=20$ en clustering, tenemos las siguientes tasas de acierto en la \hyperref[res20SURFColor]{Tabla \ref*{res20SURFColor}}. Como vemos, los resultados en este caso han empeorado con respecto al experimento anterior. Esto se debe a que al aplanar la imagen perdemos muchos detalles tales como el pistilo
...
\begin{table}[H]
  \begin{tabular}{l|r}
    ...
    \textit{RF sin bootsting} & $\qquad$ 0.49 \\
  \end{tabular}
  \label{res20SURFColor}
  \caption{Tasa de aciertos de los algoritmos usando imágenes con \textit{Color Quantization}. $k=20$ con SURF.}
\end{table}

\subsection{Usando información de los dos experimentos anteriores}

Para tamaño $k=20$ en clustering, tenemos las siguientes tasas de acierto en la \hyperref[res20SURFAmbos]{Tabla \ref*{res20SURFAmbos}}.
...
\begin{table}[H]
  \begin{tabular}{l|r}
    ...
    \textit{RF con bootsting} & $\qquad$ 0.63 \\
    \textit{RF sin bootsting} & $\qquad$ 0.65 \\
  \end{tabular}
  \label{res20SURFAmbos}
  \caption{Tasa de aciertos de los algoritmos usando la información de los dos experimentos anteriores. $k=20$ con SURF}
\end{table}
